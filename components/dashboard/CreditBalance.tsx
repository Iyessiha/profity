// ============================================================
// PROFITYX — CreditBalance : affichage solde + achat
// ============================================================
'use client'
  ...
  return (
    <div style={{ position:'relative' }}>
      ...
      <button onClick={()=>setOpen(v=>!v)} style={{ display:'flex', alignItems:'center', gap:6, background:`color-mix(in srgb, ${color} 10%, transparent)`, border:`1px solid color-mix(in srgb, ${color} 25%, transparent)`, borderRadius:7, padding:'5px 10px', cursor:'pointer', flexShrink:0 }}>
        ...
        <span style={{ fontFamily:HUD, fontSize:10, fontWeight:700, color }}>{bal}</span>
      ...
      {/* Panel */}
      {open && (
        <div style={{ position:'absolute', top:'calc(100% + 10px)', right:0, width:320, maxWidth:'calc(100vw - 20px)', background:'var(--bg1)', border:'1px solid var(--bd1)', borderRadius:12, boxShadow:`0 12px 40px var(--sh)`, zIndex:200, overflow:'hidden' }}>
          ...
          <div style={{ padding:'1rem', borderBottom:'1px solid var(--bd)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div>
              ...
              <div style={{ fontFamily:BODY, fontSize:11, color:'var(--tx3)', marginTop:3 }}>
                {data.spent} utilisés · {data.earned} gagnés au total
              </div>
            </div>
            <button onClick={()=>setOpen(false)} style={{ background:'transparent', border:'none', color:'var(--tx3)', cursor:'pointer', fontSize:18 }}>✕</button>
          </div>
              ...
              {data.packs.map(pack => (
                <div key={pack.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background:'var(--bg2)', border:`1px solid ${pack.badge?'var(--bd2)':'var(--bd)'}`, borderRadius:8, padding:'0.75rem' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <div style={{ width:36, height:36, borderRadius:7, background:'color-mix(in srgb, var(--ac) 10%, transparent)', border:'1px solid var(--bd1)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:HUD, fontSize:11, fontWeight:900, color:'var(--ac)' }}>{pack.credits}</div>
                    <div>
                  ...
                  <button onClick={()=>buy(pack)} disabled={loading===pack.id}
                    style={{ background: pack.badge?'var(--ac)':'color-mix(in srgb, var(--ac) 10%, transparent)', border:`1px solid ${pack.badge?'none':'var(--bd2)'}`, color: pack.badge?'#020408':'var(--ac)', fontFamily:HUD, fontSize:8, letterSpacing:1, fontWeight:700, padding:'7px 12px', borderRadius:4, cursor:'pointer', whiteSpace:'nowrap' }}>
